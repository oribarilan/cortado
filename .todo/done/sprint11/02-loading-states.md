...
Add loading/refresh animations to the menubar panel and the panel for all three loading scenarios: startup, manual refresh, and background refresh. Pulled from backlog item `optional-loading-animations.md`.
...
- [ ] Panel shows a comparable loading skeleton on initial load (currently may be instant or missing).
...
- Panel: only the list pane shimmers on startup. The detail pane shows its empty-state placeholder ("Select an activity") since its content depends on a selection -- shimmering it would imply something is loading there when nothing is.
